...
class mainWindow(QMainWindow):
    ...
    def setupList(self, page, cat, list):
        for i in self.downloadList[page][cat]['item']:
            index = list.model().rowCount()
            list.model().appendRow(DownloadableItem(i['title']))
            try:
                list.model().item(index).setAttrs(i['toolTip'], i['id'], i['tags'], i['url'])
            except KeyError:
                list.model().item(index).setAttrs(i['toolTip'], i['id'], i['tags'])
    ...
    def __init__(self, parent=None):
        ...
        with open('downloader.json') as f:
            j = json.load(f)
        self.downloadList = j["downloadList"]
        self.paths = j["paths"]
        self.setupAll()
